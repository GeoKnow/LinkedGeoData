...
var_dump($aDSNInfo);
                $osm2pgsql = 'PGPASS=' . $aDSNInfo['password'] . ' ';
...
echo "osm2pgsql cmd: $osm2pgsql\n";
	...
	if ($aCMDResult['index'] || $aCMDResult['all'])
	{
		...
		$sBaseCmd = CONST_BasePath.'/nominatim/nominatim -i -d '.$aDSNInfo['database'].' -P '.$aDSNInfo['port'].' -H '.$aDSNInfo['hostspec'].' -U '.$aDSNInfo['username'].' -t '.$iInstances.$sOutputFile;
	...
	function pgsqlRunScriptFile($sFilename)
	{
		...
		$aDSNInfo = DB::parseDSN(CONST_Database_DSN);
		if (!isset($aDSNInfo['port']) || !$aDSNInfo['port']) $aDSNInfo['port'] = 5432;
		$sCMD = "psql " . str_replace('pgsql', 'postgresql', CONST_Database_DSN);
