...
Create View osm_nodes_geometry As
    Construct {
        ?n geo:hasGeometry ?g .
...
Create View osm_ways As
    ...
    With
        ?w = uri(osmrw:, ?id)
        ?v = strdt(?version, xsd:int)
        ?u = uri(osmru:, ?user_id)
        ?d = strdt(?tstamp, xsd:dateTime)
        ?c = strdt(?changeset_id, xsd:int)
